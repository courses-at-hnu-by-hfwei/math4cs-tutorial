...
\begin{problem}[一阶谓词逻辑公式]
  ...
  \begin{enumerate}[(1)]
    \item 请将``Open Set''（开集）的定义翻译成一阶谓词逻辑公式,
      ...
      \begin{solution}
        取单一论域 $|\mathcal{M}|=\mathbb{R}^n$，并在量词中直接使用域限制符号。
        采用如下更紧凑的表示方法, 不额外引入专门的谓词符号:
        \
        \[\
          \phi \triangleq
          \forall x\in U\;\exists \epsilon\in \mathbb{R}^+\;\forall y\in \mathbb{R}^n\;
            \bigl(d(y,x)<\epsilon \to y\in U\bigr).
        \]
      \end{solution}
    \item 请给出 $\lnot \phi$ 公式。
      \begin{solution}
        根据上题采用的域限制量词表示, 对 $\phi$ 取否得到：
        \[
          \begin{aligned}
            \lnot\phi
            &\equiv \lnot\bigl(\forall x\in U\;\exists \epsilon\in \mathbb{R}^+\;\forall y\in \mathbb{R}^n\;
              (d(y,x)<\epsilon \to y\in U)\bigr) \\
            &\equiv \exists x\in U\;\forall \epsilon\in \mathbb{R}^+\;\exists y\in \mathbb{R}^n\;
