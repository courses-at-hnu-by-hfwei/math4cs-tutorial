...
\begin{solution}
	...
	首先，
	\[
		\emptyset\in \mathcal{T}_{\mathrm{cof}},
		\qquad
	...
	若所有 $U_i=\emptyset$，则
	\[
		\bigcup_{i\in I}U_i=\emptyset\in \mathcal{T}_{\mathrm{cof}}.
	\]

	否则，至少存在一个 $i_0\in I$ 使得 $U_{i_0}\neq \emptyset$。对每个非空 $U_i$，都有 $X\setminus U_i$ 为有限集。于是
	\[
		...
		\item 要证明 $\tau_{\sim}$ 是拓扑，只需验证三条公理。
		  ...
		  \begin{itemize}
				\item
					...
					所以
					\[
						\emptyset,\ X/{\sim}\in \tau_{\sim}.
					\]
				\item $\{U_i:i\in I\}\subseteq \tau_{\sim}$，则对每个 $i$ 都有 $q^{-1}(U_i)\in \tau$。并且
					\[
						q^{-1}\!\left(\bigcup_{i\in I}U_i\right)
						=
						\bigcup_{i\in I}q^{-1}(U_i) \in \tau.
					\]
					故
					\[
